    ...
    <Formik
        ...
        validationSchema={Yup.object().shape({
            actual_design: Yup.number()
                .transform(cv => Math.round(cv * 100) / 100)
                .min(0, 'Actual design hours cannot be negative')
                .max(9999.99, 'Actual design hours must be less than 10000')
                .required()
                .label('Actual design hours'),
            actual_development: Yup.number()
                .transform(cv => Math.round(cv * 100) / 100)
                .min(0, 'Actual development hours cannot be negative')
                .max(
                    9999.99,
                    'Actual development hours must be less than 10000',
                )
                .required()
                .label('Actual development hours'),
            actual_testing: Yup.number()
                .transform(cv => Math.round(cv * 100) / 100)
                .min(0, 'Actual testing hours cannot be negative')
                .max(9999.99, 'Actual testing hours must be less than 10000')
                .required()
